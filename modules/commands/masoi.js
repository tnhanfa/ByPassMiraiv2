...
module.exports.onLoad = async function () {
  if (!existsSync(__dirname + '/cache/werewolf.json')) {
    var { data } = await axios.get('https://raw.githubusercontent.com/HarryWakazaki/Werewolf/main/werewolf.json', { method: 'GET' });
    writeFileSync(__dirname + '/cache/werewolf.json', JSON.stringify(data,null,2), 'utf8');
  }
...
module.exports.handleReply = async function({ api,event,handleReply, Users }) {
  ...
  if (Block_Action) return api.sendMessage("[𝐇𝐋🌟] => Quá Trễ Rồi 🙉", event.threadID);
  if (handleReply.author && event.senderID != handleReply.author) return;
    else switch (parseInt(values.type)) {
      case 9: {
        switch (handleReply.type) {
          case "SoiChoose": {
            if (isNaN(event.body)) return api.sendMessage("[𝐇𝐋🌟] => Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
            if (event.body > handleReply.Listuser.length) return api.sendMessage("[𝐇𝐋🌟] => Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
            ...
            for (let i of handleReply.TeamSoi) {
              api.sendMessage('[𝐇𝐋🌟] => Ma Sói: ' + name + ' Vừa Vote ' + NanNhan + '\x0aTổng Vote: ' + Global_ArrayChoose['MaSoi'][Choose] + '/' + Values['player']['filter'](_0x32101b => _0x32101b['phe'] == 'Ma Sói')['length'], i);
            }
            api.sendMessage('[𝐇𝐋🌟] => Bạn Đã Vote: ' + NanNhan + '\x0aTổng Vote: ' + Global_ArrayChoose['MaSoi'][Choose] + '/' + Values['player']['filter'](_0x402a9e => _0x402a9e['phe'] == 'Ma Sói')['length'] + '\x0aBạn Có Thể Thay Đổi Đối Tượng', handleReply.author);
          }
          break;
          case "Another_Role_Async": {
            switch (handleReply.role) {
              case "Tiên Tri": {
                ...
                return api.sendMessage('[𝐇𝐋🌟] => Bạn Đã Chọn Tiên Tri: ' + NanNhan + ' Trong Đêm Nay, Bạn Có Thể Chọn Lại !', event.threadID);
              }
              case "Bảo Vệ": {
                if (isNaN(event.body)) return api.sendMessage("[𝐇𝐋🌟] => Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
                if (event.body > handleReply.Listuser.length) return api.sendMessage("[𝐇𝐋🌟] => Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
                ...
                return api.sendMessage('[𝐇𝐋🌟] => Bạn đã chọn bảo vệ ' + NanNhan + ', người chơi này sẽ bất tử trong đêm nay 💀, bạn có thể chọn lại !', event.threadID);
            }
          }
        } 
        case "VillageVoting": {
          if (Block_Vote) return api.sendMessage("[𝐇𝐋🌟] => Quá Trễ Rồi 🙉", event.threadID);
          if (isNaN(event.body)) return api.sendMessage("[𝐇𝐋🌟] => Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
          if (event.body > handleReply.Listuser.length) return api.sendMessage("[𝐇𝐋🌟] => Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
          if (!handleReply.Listuser.some(i => i.id == event.senderID)) return api.sendMessage("[𝐇𝐋🌟] => Bạn Không Có Quyền Vote !", event.threadID);
          var Choose = handleReply.Listuser[event.body - 1];
          ...
          api.sendMessage('[𝐇𝐋🌟] => Vote: ' + name + '(' + event['body'] + ') Thành Công ! \x0aTổng Vote: ' + Global_ArrayChoose['DanLang'][Choose['id']] + '/' + (handleReply['Listuser']['length'] - SoiT['length'] - 0x1), event.threadID);
        }
        break;
      }
    }
...
module.exports.MaSoiChoose = async function(api, event, Users,Data, TeamSoi) {
  ...
  for (let i of sus) {
    ...
    var Objection = "";
    var stt = 1;
    api.sendMessage("[𝐇𝐋🌟] => Phản Hồi Và Chọn 1 trong (tin nhắn chứa) các tên được liệt kê dưới đây, chú ý : bản cần chọn đúng và chỉ đc chọn 1 lần",i.id);
...
module.exports.Another_Role_Async = async function(api, event, Users,Data) {
  ...
  var playerfilter = values.player;
    for (let i of playerfilter) {
      switch (i.vai) {
        case "Dân Làng": {
          api.sendMessage('[𝐇𝐋🌟] => Không Có Việc Gì Làm, Đi Ngủ Thôi 🐧',i.id);
        }
        break;
        case "Tiên Tri": {
          let Player = [];
          ...
          api.sendMessage('[𝐇𝐋🌟] => Hãy Chọn 1 Trong Những Đối Tượng Dưới Đây Để Xem Có Phải Là Sói Hay Không !', i.id);
        api.sendMessage(Objection,i.id, (err,info) => global.client.handleReply.push({
      ...
      case "Bảo Vệ": {
        ...
        api.sendMessage('[𝐇𝐋🌟] => Hãy Chọn 1 Trong Những Đối Tượng Dưới Đây Để Bảo Vệ !', i.id);
        api.sendMessage(Objection,i.id, (err,info) => global.client.handleReply.push({
          type: "Another_Role_Async",
          name: this.config.name,
          author: i.id,
...
module.exports.VillageLogic_Sort = async function(api, event, Users,Data,TeamSoi) {
  ...
  if (list.length < 1) {
    ...
    api.sendMessage('[𝐇𝐋🌟] => Người Bị Treo Cổ: ' + name + '\x0aChức Năng: ' + Find['vai'], event.threadID);
...
module.exports.EndGame = async function(api, event,Data,winner) {
  switch (winner) {
    case "Village": {
      ...
      return api.sendMessage('[𝐇𝐋🌟] => Tổng Ngày Trôi Qua: ' + Days + '\x0aTổng Người Còn Sống: ' + All['length'] + '\x0a\x0a' + Objection,event.threadID);
    }
    case "Werewolf": {
...
module.exports.Logic_Sort = async function(api, event, Users,Data, TeamSoi) {
  ...
  if (Global_ArrayChoose.MaSoi.hasOwnProperty(Global_ArrayChoose.BaoVe[0])) {
    if (Object.keys(Global_ArrayChoose.MaSoi).length == 2) {
      ...
      for (let i of TeamSoi) {
        api.sendMessage(`[𝐇𝐋🌟] => Vì Không Cùng Mục Tiêu, Nên Hệ Thống Sẽ Random Đối Tượng Mà 2 Bên Đã Chọn !`,i);
        api.sendMessage(`[𝐇𝐋🌟] => Đối Tượng Được Chọn Là: ${name}`,i);
      }
      if (Global_ArrayChoose.MaSoi.hasOwnProperty(Global_ArrayChoose.BaoVe[0])) {
        var values = Data.get(global.AuthThread)
        var find = values.player.find(i => i.vai == "Bảo Vệ");
        for (let i of TeamSoi) {
          api.sendMessage(`[𝐇𝐋🌟] => ${name} Đã Bị Bảo Vệ !`,i, (err,info) => { Global_ArrayChoose.MaSoi = {}; });
        }
        api.sendMessage(`[𝐇𝐋🌟] => Bạn Vừa Bảo Vệ ${name} Thành Công !`, find.id); 
      }
      else {
        for (let i of TeamSoi) {
          api.sendMessage(`[𝐇𝐋🌟] => Đã Thủ Tiêu Thành Công: ${name}`,i, (err,info) => { Global_ArrayChoose.MaSoi = {};});
        }
        DataGM.Die = name; 
        var values  = Data.get(global.AuthThread)
        values.player.splice(values.player.findIndex(i => i.id == RandomTarget),1);
        return api.sendMessage('[𝐇𝐋🌟] => Bạn Đã Bị Ma Sói Thủ Tiêu Tối Qua !', RandomTarget);
      }
    }
    ...
    if (Object.keys(Global_ArrayChoose.MaSoi).length == 0 || Object.keys(Global_ArrayChoose.MaSoi).length == 1 && Global_ArrayChoose.MaSoi[Object.keys(Global_ArrayChoose.MaSoi)[0]] < WereWolf.length) {
      for (let i of TeamSoi) {
        api.sendMessage(`[𝐇𝐋🌟] => Không Có Đối Tượng Nào Được Chọn Hoặc Không Đủ Vote !`,i);
      }
    }
    else {
      if (Object.keys(Global_ArrayChoose.MaSoi).length == 2) {
        ...
        return api.sendMessage('[𝐇𝐋🌟] => Bạn Đã Bị Ma Sói Thủ Tiêu Tối Qua !', Object.keys(Global_ArrayChoose.MaSoi)[0]);
      }
    }
  }
  //tiên tri
  if (getType(Global_ArrayChoose.TienTri.Choose) == "Object" && Global_ArrayChoose.TienTri.Choose != "String") {
    ...
    api.sendMessage('[𝐇𝐋🌟] => Chức Vụ Của ' + name + " Là: " + Global_ArrayChoose.TienTri.Choose.role, Global_ArrayChoose.TienTri.Owner);
  }
}
...
async function start(api,event,Users,Data,TeamSoi) {
  await new Promise(resolve => setTimeout(resolve, 3*1000));
    var out = async(msg) => api.sendMessage("[𝐇𝐋🌟] => " + msg,event.threadID);
  try {
    out("[𝐇𝐋🌟] => Màn đêm bắt đầu buông xuống !, bây giờ là thời gian của sói hoạt động...");
  ...
  case "CHIA VAI":
        ...
        case 9: {
          var role = ["Dân Làng","Sói thường","Tiên Tri","Bảo Vệ","Sói thường","Dân Làng","Dân Làng","Dân Làng","Dân Làng"];
            for (let i = 0;i < values.player.length;i++) {
              ...
              api.sendMessage({ body:  '[𝐇𝐋🌟] => Vai Trò Của Bạn Là: ' + is['Name'] + '\x0aChi Tiết: ' + is['Description'] + '\x0aThuộc Phe: ' + is['Type'] + ' !',attachment: createReadStream(join(__dirname, is.File))}, values.player[i].id,(ee,inf) => { if (ee) { LOI.push(values.player[i].id);} });
            }
          values.phanvai = 1;global.moduleData.werewolf.set(threadID, values);
            api.sendMessage("[𝐇𝐋🌟] => Đã Phân Vai Thành Công !, Hãy Kiểm Tra Tin Nhắn Riêng Của Bot, Lưu Ý Nếu Acc Dưới 18 Tuổi Không Được Tham Gia !",event.threadID, event.messageID);
              if (LOI.length != 0) { 
                ...
                return api.sendMessage("[𝐇𝐋🌟] => Không gửi Được Tin Nhắn Đồng Nghĩa Player Đã Chặn Bot Hoặc Acc Dưới 18+,Game Sẽ Không Thực Hiện Được, Tiến Hành Restart Bot, Hủy Game Tránh Lỗi ( bấm werewolf help để biết chi tiết )",threadID, (err, info) => { return process.exit(1); });
              }
            if (Team1.length != 0) {
              try {
                for (let check of Team1) {
                  if (check == Team1[0]) {
                    ...
                    api.sendMessage("[𝐇𝐋🌟] => Đồng Đội Của Bạn Là : " + name + ", ID: " + Team1[1] + "\nHãy Nhắn Tin Với Nhau Để Hợp Tác Tốt Nhất !",check);
                  }
                    ...
                    api.sendMessage("[𝐇𝐋🌟] => Đồng Đội Của Bạn Là : " + name + ", UID: " + Team1[0] + "\nHãy Nhắn Tin Với Nhau Để Hợp Tác Tốt Nhất !",check);
                  }
                }
                return await start(api,event,Users,global.moduleData.werewolf,Team1);
              }
              catch (e) { console.log(e); return api.sendMessage('[𝐇𝐋🌟] => Đã Lỗi !',event.threadID);}
            }
          }
        } 
      };
    break;
      case "TEST": {
        var test = [];
          for (let i = 0;i < values.player.length;i++) {
            api.sendMessage('[𝐇𝐋🌟] => Bạn Có Thấy Tin Nhắn Này ?',values.player[i].id,(error,info) => {if (error) { test.push(values.player[i].id);}});
          }
        if (test.length != 0) {
          for (let kan of test) {
            var name = (await api.getUserInfoV5(kan))[0].o0.data.messaging_actors[0].name || (await Users.getInfo(kan)).name;
            api.sendMessage("[𝐇𝐋🌟] => Phát Hiện Acc Dưới 18+ Hoặc Đã Block Acc Bot" + " Tại User: " + name + ", Tiến Hành Restart Bot, Hủy Game Tránh Lỗi,Chi Tiết Tại werewolf help",event.threadID);
          }
          return process.exit(1);
        }
        else return api.sendMessage("[𝐇𝐋🌟] => Không Phát Hiện Acc Dưới 18+ Hoặc Đã Block Acc Bot",event.threadID);
      }
      break;
      case "PING": {
        return api.sendMessage(`[𝐇𝐋🌟] => Tổng Số Người Tham Gia: ${values.player.length}, Đã Chia Phe: ${values.phanvai==1? "True": "False"}, Đã Start Game: ${values.start==1?"True": "False"}`,event.threadID);
      }
    default: {}
  }
};
            ...
            case "Create": {
              if (!global.moduleData.werewolf) global.moduleData.werewolf = new Map();
                var values = global.moduleData.werewolf.get(event.threadID) || {};
                  if (global.moduleData.werewolf.has(event.threadID)) return api.sendMessage("[𝐇𝐋🌟] => Nhóm đang có bàn ma sói !", threadID, messageID);
              ...
              return out("[𝐇𝐋🌟] => Đã Tạo Thành Công Bàn Có 8-9 Người! Hãy bấm masoi join để tham gia !, Hướng Dẫn Tại masoi help!, Lưu Ý Không Được Cho Acc Dưới 18 Tuổi ( Facebook ) Chơi Vì Sẽ Lỗi !"); 
            }
          case "check":
            case "Check": {
              var values = global.moduleData.werewolf.get(event.threadID) || {};
              return out(`[𝐇𝐋🌟] => Status: ${values.player.length}/${values.type}`);
            }
        }
      }
        break;
      case "10-11": {
        return out("[𝐇𝐋🌟] => Chưa Hoàn Thành !");
      }
      case "12-13": {
        return out("[𝐇𝐋🌟] => Chưa Hoàn Thành !");
      } 
      ...
      case "join":
        case "Join": {
          var values = global.moduleData.werewolf.get(event.threadID) || {};if (!values.player) return out('Tạo Phòng Đê') ;if (values.player.length >= values.type) return out("Phòng Đã Đầy !");
            if (values.player.find(item => item.id == senderID)) return api.sendMessage("[𝐇𝐋🌟] => Bạn Đã Tham Gia Rồi !", event.threadID, event.messageID);
              if (!values) return api.sendMessage("[𝐇𝐋🌟] => Hiện Tại Chưa Có Ván Ma Sói Nào Được Mở!", event.threadID, event.messageID);
                if (values.start == 1) return api.sendMessage("[𝐇𝐋🌟] => Chin Nhỗi Nhưng Ván Ma Sói Của Nhóm Này Đã Start !", threadID, messageID);
              values.player.push({ "id": senderID, "vai": 0, "phe": 0,"ready": false, "vote": 0 });
            global.moduleData.werewolf.set(threadID, values);
          return api.sendMessage(`[𝐇𝐋🌟] => Status: ${values.player.length}/${values.type}`,threadID)
        }
      case "leave":
        case "Leave": {
          var values = global.moduleData.werewolf.get(event.threadID) || {};
            if (typeof values.player == "undefined") return api.sendMessage("[𝐇𝐋🌟] => Hãy Tạo Ván Ma Sói Bằng Lệnh masoi số người tham gia :[8-9] Create !", event.threadID, event.messageID);
              if (!values.player.some(item => item.id == senderID)) return api.sendMessage("[𝐇𝐋🌟] => Bạn chưa tham gia vào bàn ma sói trong nhóm này!", event.threadID, event.messageID);
                if (values.start == 1) return api.sendMessage("[𝐇𝐋🌟] => Chin Nhỗi Nhưng Ván Ma Sói Của Nhóm Này Đã Start !", threadID, messageID);
          ...
          var values = global.moduleData.werewolf.get(event.threadID) || {}; 
            if (!values) return api.sendMessage("[𝐇𝐋🌟] => Hiện Tại Chưa Có Ván Ma Sói Nào Được Mở!", event.threadID, event.messageID);
              if (senderID == values.author) {
                if (values.player.length <= 1 || values.player.length != values.type) return api.sendMessage(`[𝐇𝐋🌟] => Đang Thiếu Người, Hiện Tại Có : ${values.player.length}/${values.type} Người !`, threadID, messageID);
              if (values.start == 1) return api.sendMessage("[𝐇𝐋🌟] => Đã Bắt Đầu Rồi !", threadID, messageID);
            values.start = 1;
          return out("[𝐇𝐋🌟] => Bắt Đầu Thành Công!");
        }
      }
        break;
      case "help":
        case "Help": return out("[𝐇𝐋🌟] => Các Loại: masoi [9,status,leave,join]\nTạo Bàn: masoi 9 create\nKiểm Tra: masoi 9 check\nRời Bàn: masoi leave\nTham Gia: masoi join\nBắt Đầu: masoi start\nLưu Ý: Không Được Cho Acc Dưới 18 Tuổi ( Facebook ) Chơi Vì Sẽ Lỗi !");
    default: return out("[𝐇𝐋🌟] => Các Loại: masoi [9,status,leave,join]\nTạo Bàn: masoi 9 create\nKiểm Tra: masoi 9 check\nRời Bàn: masoi leave\nTham Gia: masoi join\nBắt Đầu: masoi start\nLưu Ý: Không Được Cho Acc Dưới 18 Tuổi ( Facebook ) Chơi Vì Sẽ Lỗi !");
  }
};